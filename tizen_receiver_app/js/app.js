...
const DEFAULT_BASE_URL = "http://10.171.64.186:65331";
  ...
  try {
    ...
    if (stored && Array.isArray(stored.entries)) {
      offlineLibrary = {
        ...
        entries: stored.entries.filter((entry) => entry && entry.id && entry.content_hash),
...
async function syncOfflineLibrary(baseUrl, alias) {
  ...
  offlineLibrarySyncPromise = (async () => {
    const manifest = await fetchLibraryManifestOnce(baseUrl, alias);
    const requestId = String(manifest.request_id || "");
    const revision = String(manifest.revision || "");
    if (!requestId) {
      return;
    }
    if (requestId === offlineLibrary.failedRequestId) {
      await reportLibraryStatus(
        baseUrl,
        manifest,
        "failed",
        "The previous attempt for this request failed. Request another offline-library sync to try again.",
      );
      return;
    }
  ...
  try {
    await offlineLibrarySyncPromise;
  } finally {
...
function renderSourceMenu() {
  ...
  for (let offset = 0; offset < visible; offset += 1) {
    ...
    item.className = `source-menu-item${index === sourceMenuIndex ? " selected" : ""}${entry.playable ? "" : " unsupported"}`;
...
function openSourceMenu() {
  if (!offlineLibrary.entries.length) {
    showRemoteFeedback("No saved sources are available on this TV yet.");
    return;
  }
