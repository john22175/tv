...
1. Copy `deploy.targets.example.json` to `deploy.targets.json` and enter the public `sourceRepository`, each TV's host, SDB serial, dashboard `receiverId` (`tv-1` through `tv-6`), and Tizen certificate profile.
